...
class AutomatonComponent extends Component {
	...
	public AutomatonComponent(ICellAutomaton automaton) {
		...
		gridDim = new Dimension((cellWidth + padding) * automaton.getWidth() + padding,
				(cellHeight + padding) * automaton.getHeight() + padding);
	}
	...
	public void paint(Graphics g) {
		for (int x = 0; x < automaton.getWidth(); x++) {
			for (int y = 0; y < automaton.getHeight(); y++) {
				...
				g.fillRect(x * (cellHeight + padding) + padding, y * (cellHeight + padding) + padding, cellHeight,
						cellWidth);
			}
		}
	}
